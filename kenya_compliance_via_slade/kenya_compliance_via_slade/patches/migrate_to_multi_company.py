...
def execute() -> None:
    ...
    setups = frappe.get_all(
        SETTINGS_DOCTYPE_NAME, filters={"is_active": 1}, fields=["name", "company"]
    )
    ...
    if len(setups) == 1:
        ...
        update_entities(
            setup, "Supplier", "custom_details_submitted_successfully", "slade_id"
        )
...
def update_entities(setup, doctype, filter_field, id_field):
    registered_entities = frappe.get_all(
        doctype, filters={filter_field: 1}, fields=["name", id_field]
    )

    for entity in registered_entities:
        mapping_exists = frappe.db.exists(
            SLADE_ID_MAPPING_DOCTYPE_NAME,
            {"parent": entity.name, "parenttype": doctype, "setup_docname": setup.name},
        )
        ...
        if mapping_exists:
            frappe.db.set_value(
                SLADE_ID_MAPPING_DOCTYPE_NAME,
                mapping_exists,
                {"etims_id": entity.get(id_field), "is_active": 1},
            )
        else:
            new_mapping = {
                ...
                "etims_id": entity.get(id_field),
                "setup_docname": setup.name,
                "is_active": 1,
            }
            frappe.get_doc(new_mapping).insert(
